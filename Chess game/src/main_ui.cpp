    ...
    for (int row = 0; row < 8; row++) {
        for (int col = 0; col < 8; col++) {
            ...
            if (piece != nullptr) {
                ...
                if (idx != std::string::npos && textures[idx].id != 0) {
                    float texW = textures[idx].width;
                    float texH = textures[idx].height;
                    float cellSize = CELL - 8;

                    // Tính scale giữ tỉ lệ
                    float scale = (texW > texH) ? cellSize / texW : cellSize / texH;
                    float drawW = texW * scale;
                    float drawH = texH * scale;

                    // Căn giữa trong ô
                    float drawX = BOARD_X + col * CELL + (CELL - drawW) / 2;
                    float drawY = BOARD_Y + row * CELL + (CELL - drawH) / 2;
...
void drawSidebar(bool isWhiteTurn, UIState& ui) {
    ...
    Color turnColor;
    if (isWhiteTurn) turnColor = WHITE;
    else turnColor = { 40, 40, 40, 255 };
    DrawText(turnText, sx + 20, 100, 28, turnColor);

    // Chỉ báo màu lượt (hình tròn)
    Color circleColor;
    if (isWhiteTurn) circleColor = WHITE;
    else circleColor = { 40, 40, 40, 255 };
